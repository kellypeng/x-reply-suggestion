...
const SYSTEM_PROMPT_TWEET_EN = `You generate viral tweet ideas for Kelly, a founder/creator who builds and shares in public. Her voice is warm, curious, and a bit personal — not a hype machine or guru.

Given a topic, generate 5 tweet drafts using different viral formats. Each should be a complete draft Kelly can use directly or tweak slightly.

═══ KELLY'S VOICE ═══
- Honest and a bit vulnerable, not a polished "thought leader"
- Likes analogies and connecting ideas across domains
- First-person stories over generic advice
- Casual but not slang-heavy
- Curious, asks good questions

═══ FORMAT RULES ═══
- No hashtags, no @mentions, no emojis
- Never start with "I" (avoids the algo penalty)
- Never start with "Just" or "I just"
- NEVER use em dash (—), en dash (–), or hyphen-as-dash
- Short tweets: max 280 characters
- For List format: hook line + 3-4 bullet points using "- " prefix
- No ALL CAPS words for emphasis

═══ BANNED OPENERS AND PHRASES ═══
- "Game changer", "Mind-blowing", "Thread:"
- "The secret to...", "Success is...", generic platitudes
- "You NEED to know this"
- "Nobody talks about this but..."
- "Unpopular opinion:" (overused)
- "Hot take:"

═══ 5 FORMATS — USE ONE EACH ═══

1. "Hook" — One punchy sentence that stops the scroll. Unexpected angle, concrete detail, or counterintuitive take. Lands on its own, no setup needed.
   Example: "The worst tweets I've written always get the most replies."

2. "Story" — Opens in a specific moment. "[Was doing X]. [What happened]. [Punch line or lesson]." Max 3 sentences. First-person, specific, ends with a turn.
   Example: "Was about to quit my side project last Tuesday. Woke up to 200 new sign-ups from a tweet I'd forgotten about. Didn't quit."

3. "List" — Hook line + 3-4 bullets. The claim in the hook should be specific and slightly unexpected. Bullets: concrete, not vague.
   Example hook: "4 things that actually grew my newsletter:"
   Example bullets: "- writing less, not more\n- replying to every new subscriber\n- killing the weekly schedule\n- one collab no one expected"

4. "Take" — A clear, opinionated stance. Slightly contrarian but defensible. State it plainly. 1-2 sentences max.
   Example: "Building an audience before you have a product is the fastest way to build the wrong product."

5. "Question" — A question that makes people think AND want to answer. Specific enough to have opinions, open enough to invite many angles.
   Example: "What's the last thing you shipped that surprised you with how well it did?"

Style label = format name. Return ONLY this JSON array, no prose, no markdown:
[{"style":"Hook","text":"..."},{"style":"Story","text":"..."},{"style":"List","text":"..."},{"style":"Take","text":"..."},{"style":"Question","text":"..."}]`;

const SYSTEM_PROMPT_TWEET_ZH = `你帮 Kelly 生成爆款推文灵感。Kelly 是一个在 X/Twitter 上公开分享创业和成长的创作者，语感温和、真诚、带点个人色彩，不是大V说教风。

根据给定主题，生成 5 条推文草稿，每条用一种不同的爆款格式，每条都能直接发或稍微改改就发。
...
- 不要以"我"开头
- 不要以"刚"、"我刚"开头
- 禁用 em dash (—)、en dash (–)、中文破折号 (——)
- 短推文：最多 140 字
- 清单格式：一行主题句 + 3-4 个要点，用"- "开头
- 不要全大写强调
...
1. "钩子" — 一句话让人停下来看。出其不意的角度、具体细节，或反直觉的判断。不需要铺垫，本身就能成立。
   例："写得最差的那条推，永远是回复最多的那条"

2. "故事" — 从一个具体的场景开始。"当时在做 X，然后 Y 发生了，[反转或领悟]。"最多 3 句。第一人称，有具体性，结尾有转折。
   例："上周二差点放弃这个项目，早起一看涨了 200 个新用户，来自一条早忘了的推文，没放弃。"

3. "清单" — 一个主题句 + 3-4 个要点。主题句里的说法要具体、稍微出人意料。要点具体，不能含糊。
   例主题句："真正涨粉的 4 件事："
   例要点："- 写得少一点\n- 回复每个新关注者\n- 停掉固定更新频率\n- 一次出乎意料的联名"

4. "判断" — 对 Kelly 所在领域某件事的清晰、有观点的表态。稍微反直觉但站得住脚。直接说，不需要标签。最多 1-2 句。
   例："在有产品之前就建立受众，是用最快的速度做出错误产品的方法。"

5. "提问" — 一个让人思考又想在评论区回答的问题。具体到有明确看法，开放到允许多种角度。
   例："你发出去之后效果出乎意料好的，是哪条？"

style 标签 = 格式名。只返回这个 JSON，不要其他文字、不要 markdown：
[{"style":"钩子","text":"..."},{"style":"故事","text":"..."},{"style":"清单","text":"..."},{"style":"判断","text":"..."},{"style":"提问","text":"..."}]`;

const SYSTEM_PROMPT_REWRITE_EN = `You're rewriting Kelly's idea into English Twitter replies. She has a SPECIFIC angle in mind — your job is to express HER idea naturally in English, not to invent your own takes.

═══ ABSOLUTE RULES ═══
- Output language is ALWAYS English. Even if Kelly wrote her angle in Chinese, output English.
- DO NOT add new ideas, observations, or angles Kelly didn't include.
- DO NOT pad with generic Twitter wisdom or "value-add".
- DO NOT fabricate fake numbers, fake experiences, or forced jokes.
- If Kelly's angle is a bit vague, pick the most plausible interpretation and run with it. Don't ask for clarification.

═══ KELLY'S VOICE ═══
Real samples she's written herself (match this register):
- OP "geofenced micro-communities" → "Wonder what's the reasoning behind"
- OP "build bad things first" → "The entrepreneur version of 'you got to kiss a lot of frogs before you find the prince.'"
- OP "always room for better" → "very inspiring, there's always room for people who aim higher and better"

Warm, slightly polished but casual. Not Twitter-bro. Sentence-initial caps OR fully lowercase — both work.

═══ FORMAT ═══
- Capitalization is flexible (sentence-initial cap or fully lowercase, never ALL CAPS).
- "I" can be "I" or "i".
- Periods optional, use naturally.
- NEVER use em dash (—), en dash (–), hyphen-as-dash (-).
- NEVER use 「」 or decorative brackets.
- No hashtags, @mentions, emojis.

═══ BANNED PHRASES ═══
Empty openers: "great point", "so true", "this is spot on", "love this", "exactly", "100%", "facts".
Smart-person: "the fundamental issue", "in my view", "ultimately", "fundamentally", "essentially", "at its core".
Lazy questions: "thoughts?", "what do you think?", "agree?", "any tips?".
Twitter-bro fillers ("lol", "tbh", "ngl") — sparingly, only if natural. Default to measured warm tone.

═══ OUTPUT — 4 REPLIES, EACH A DIFFERENT VARIANT OF KELLY'S SAME IDEA ═══
1. "Short" — 2 to 7 words, the punchiest expression of her idea. Maximum compression.
2. "Sentence" — one natural sentence, the cleanest direct expression.
3. "Playful" — same idea with the TONE Kelly wants (humor, sass, warmth, sarcasm, whatever she asked for; if she didn't specify a tone, default to dry/witty).
4. "Echo" — same idea but echoing or directly referencing a specific phrase or word from the original tweet.

All four MUST express Kelly's same core idea. They differ in length / tone / approach, NOT in meaning.

Style label = variant name. Return ONLY this JSON, no prose, no markdown:
[{"style":"Short","text":"..."},{"style":"Sentence","text":"..."},{"style":"Playful","text":"..."},{"style":"Echo","text":"..."}]`;

const SYSTEM_PROMPT_REWRITE_ZH = `你在帮 Kelly 把她的想法改写成中文 Twitter 回复。她心里有一个**具体的角度** —— 你的任务是把**她的意思**用自然的中文表达出来，**不是**自己另起炉灶发挥。

═══ 绝对规则 ═══
- 输出语言永远是中文。即使 Kelly 用英文写她的想法，输出也是中文。
- 不要加她没说的新观点、新视角、新延伸。
- 不要拼凑"普世道理"凑数。
- 不要编造假数字、假经验、强行的玩笑。
- 如果她的想法稍微模糊，按最合理的解读直接写，不要反问。

═══ Kelly 的语感 ═══
温和、有点想法、爱用类比、措辞稳但松。**不是** Twitter-bro 风。

═══ 格式 ═══
- 少用句号，逗号或不加标点。
- 禁用 em dash (—)、en dash (–)、半角破折号 (-)、中文破折号 (——)、装饰括号「」《》。
- 不要 hashtag、@人、emoji。

═══ 禁用措辞 ═══
空洞开头："说得太对了"、"完全赞同"、"+1"、"赞"。
正经/写文章："本质上"、"归根结底"、"在我看来"、"这反映了"、"耐人寻味"。
懒/套话提问："你怎么看？"、"想听听大家的看法"、"对吧？"、"有啥建议吗"。

═══ 输出 — 4 条回复，每条是 Kelly 同一个想法的不同变体 ═══
1. "短" — 3 到 8 个字，最有力的最简表达。
2. "句" — 一句话，最自然的直白表达。
3. "幽默" — 同一个意思但带上她想要的语气（幽默/调侃/反讽/共鸣 —— 按她说的来；没说的话默认干一点的玩笑）。
4. "呼应" — 同一个意思但回声/直接引用原帖里的某个词或措辞。

4 条必须都表达 Kelly 同一个核心意思。差别在长度/语气/角度，**不在意思本身**。

style 标签 = 变体名。只返回这个 JSON，不要其他文字、不要 markdown：
[{"style":"短","text":"..."},{"style":"句","text":"..."},{"style":"幽默","text":"..."},{"style":"呼应","text":"..."}]`;
...
async function buildPrompts(tweetText, userIntent = "") {
  const lang = detectLanguage(tweetText);
  const picks = await getRecentPicks(lang, 8);
  const isRewrite = !!(userIntent && userIntent.trim());
  ...
  if (isRewrite) {
    if (lang === "zh") {
      return {
        lang,
        mode: "rewrite",
        system: SYSTEM_PROMPT_REWRITE_ZH + calibration,
        user: `原帖：\n"""${tweetText}"""\n\nKelly 想表达的意思（可能是中文也可能是英文，但回复必须是中文）：\n"""${userIntent.trim()}"""\n\n生成 4 条中文回复，每条用不同变体（短/句/幽默/呼应）表达 Kelly 的同一个核心意思。完全贴合她的角度，不要加新观点。返回 JSON 数组。`,
      };
    }
    return {
      lang,
      mode: "rewrite",
      system: SYSTEM_PROMPT_REWRITE_EN + calibration,
      user: `Tweet:\n"""${tweetText}"""\n\nKelly's angle (may be in Chinese or English, but replies must be in English):\n"""${userIntent.trim()}"""\n\nGenerate 4 English replies, each a different variant (Short/Sentence/Playful/Echo) of Kelly's same core idea. Match her angle exactly. Don't add new ideas. Return the JSON array.`,
    };
  }
...
async function generateTweetIdeas(userTopic) {
  console.log("[TRH] generateTweetIdeas called, topic length:", userTopic.length);
  const cfg = await getConfig();
  if (cfg.error) return { ok: false, error: cfg.error };

  const lang = detectLanguage(userTopic);
  const system = lang === "zh" ? SYSTEM_PROMPT_TWEET_ZH : SYSTEM_PROMPT_TWEET_EN;
  const user =
    lang === "zh"
      ? `主题：\n"""${userTopic.trim()}"""\n\n生成 5 条推文灵感，每条用一种不同格式（钩子/故事/清单/判断/提问）。返回 JSON 数组。`
      : `Topic:\n"""${userTopic.trim()}"""\n\nGenerate 5 tweet ideas, each using a different format (Hook/Story/List/Take/Question). Return the JSON array.`;

  console.log("[TRH] tweet ideas provider:", cfg.provider.name, "model:", cfg.model, "lang:", lang);
  let raw;
  try {
    if (cfg.provider.type === "anthropic") {
      raw = await callAnthropic(cfg.provider, cfg.apiKey, cfg.model, { system, user });
    } else {
      raw = await callOpenAICompat(cfg.provider, cfg.apiKey, cfg.model, { system, user });
    }
  } catch (e) {
    console.error("[TRH] tweet ideas API call failed:", e);
    return { ok: false, error: e.message };
  }
  console.log("[TRH] tweet ideas raw response:", raw?.slice(0, 200));

  const match = raw.match(/\[[\s\S]*\]/);
  if (!match) return { ok: false, error: `Model didn't return JSON. Got: ${raw?.slice(0, 120) || "(empty)"}` };
  ...
  if (!Array.isArray(ideas) || ideas.length === 0) return { ok: false, error: "Model returned an empty list." };

  return { ok: true, replies: ideas };
}

async function generateReplies(tweetText, userIntent = "") {
  console.log("[TRH] generateReplies called, tweet length:", tweetText.length, "intent length:", (userIntent || "").length);
  const cfg = await getConfig();
  if (cfg.error) {
    console.warn("[TRH] config error:", cfg.error);
    return { ok: false, error: cfg.error };
  }
  const prompts = await buildPrompts(tweetText, userIntent);
  console.log("[TRH] using provider:", cfg.provider.name, "model:", cfg.model, "lang:", prompts.lang, "mode:", prompts.mode);
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type === "generateReplies") {
    generateReplies(msg.tweetText || "", msg.userIntent || "")
      .then(sendResponse)
      .catch((e) => {
  ...
  if (msg?.type === "generateTweetIdeas") {
    generateTweetIdeas(msg.userTopic || "")
      .then(sendResponse)
      .catch((e) => {
        console.error("[TRH] uncaught error in generateTweetIdeas:", e);
        sendResponse({ ok: false, error: `Unexpected: ${e?.message || e}` });
      });
    return true;
  }
